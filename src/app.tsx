...
export default function App() {
  ...
  const handleSelectTask = useCallback((_task: Task) => {
  ...
  const handleSubmitTask = useCallback(
    (taskData: { title: string; description?: string; priority: 'low' | 'medium' | 'high' }) => {
      const newTask: Task = {
        ...taskData,
        status: 'todo',
        id: generateId(),
  ...
  return (
    <Box flexDirection="column" height="100%">
      ...
      <Box flexDirection="column" flexGrow={1}>
        ...
        {view === 'add' && <TaskForm onSubmit={handleSubmitTask} onBack={handleBackToMenu} />}

        {view === 'search' && (
          <SearchPanel tasks={tasks} onSelectTask={handleSelectTask} onBack={handleBackToMenu} />
        )}

        {view === 'stats' && <StatsPanel tasks={tasks} onBack={handleBackToMenu} />}
      </Box>
