...
const TaskRow = ({ task }: { task: Task }) => {
  ...
  return (
    <Box gap={1}>
      ...
      <Text
        color={task.status === 'todo' ? 'yellow' : task.status === 'in-progress' ? 'blue' : 'green'}
      >
        {statusStr}
      </Text>
      <Text
...
program
  ...
  .action((id, options) => {
    ...
    if (options.force) {
      ...
      if (deletedTask) {
        renderAndExit(<SuccessMessage text={`Task deleted: ${deletedTask.title}`} />)
      } else {
        renderAndExit(<ErrorMessage text="Error: Task not found" />)
      }
    } else {
      // For non-interactive delete, we need to handle confirmation
      // For now, require -f flag in single-command mode
      renderAndExit(<ErrorMessage text="Use -f flag to force deletion in single-command mode" />)
    }
  })
...
program
  ...
  .action((id, options) => {
    ...
    const task: Task = {
      ...existingTask,
      title: options.title ?? existingTask.title,
      description: options.description ?? existingTask.description,
      status:
        options.status && isValidTaskStatus(options.status) ? options.status : existingTask.status,
      priority:
        options.priority && isValidTaskPriority(options.priority)
          ? options.priority
...
// Search command
program.command('search <query>').action((query) => {
  const tasks = loadTasks()
  ...
  const StatsComponent = () => {
    ...
    return (
      <Box flexDirection="column" gap={1}>
        ...
        <Text>
          Total: <Text color="cyan">{total}</Text> task{total !== 1 ? 's' : ''}
        </Text>
        <Box flexDirection="column" gap={0} marginTop={1}>
          <Text bold>By Status:</Text>
          <Text>
            {' '}
            ○ <Text color="yellow">Todo</Text>: {todo}
          </Text>
          <Text>
            {' '}
            ◐ <Text color="blue">In Progress</Text>: {inProgress}
          </Text>
          <Text>
            {' '}
            ● <Text color="green">Done</Text>: {done}
          </Text>
        </Box>
        <Box flexDirection="column" gap={0} marginTop={1}>
          ...
          <Text>
            {' '}
            ▲ <Text color="red">High</Text>: {high}
          </Text>
          <Text>
            {' '}
            ● <Text color="yellow">Medium</Text>: {medium}
          </Text>
          <Text>
            {' '}
            ▽ <Text color="gray">Low</Text>: {low}
          </Text>
        </Box>
        <Box marginTop={1}>
          <Text>
            Last updated: <Text bold>{lastTask.title}</Text> (
            <Text color="gray">{lastTask.updatedAt.slice(0, 10)}</Text>)
          </Text>
        </Box>
      </Box>
    )
